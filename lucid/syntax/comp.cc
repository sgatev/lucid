...
std::expected<void, CompError> CheckCompExpr(
  ...
  if (const auto* func_call_expr = std::get_if<FuncCallExpr>(&expr)) {
    ...
    return CheckCompExpr(sctx, func_defs, index_expr->base).and_then([&] {
      return CheckCompExpr(sctx, func_defs, index_expr->index);
    });
  } else if (const auto* binary_op_expr = std::get_if<BinaryOpExpr>(&expr)) {
    return CheckCompExpr(sctx, func_defs, binary_op_expr->lhs).and_then([&] {
      return CheckCompExpr(sctx, func_defs, binary_op_expr->rhs);
    });
  }
  return {};
}
...
std::expected<void, CompError> CheckCompFunc(
  ...
  if (const auto* if_stmt = std::get_if<IfStmt>(&stmt)) {
    return CheckCompFunc(sctx, func_defs, if_stmt->then_stmts).and_then([&] {
      return CheckCompFunc(sctx, func_defs, if_stmt->else_stmts);
    });
  } else if (const auto* loop_stmt = std::get_if<LoopStmt>(&stmt)) {
    return CheckCompFunc(sctx, func_defs, loop_stmt->stmts);
  } else if (const auto* var_decl_stmt = std::get_if<VarDeclStmt>(&stmt)) {
    if (var_decl_stmt->init.has_value()) {
      return CheckCompExpr(sctx, func_defs, *var_decl_stmt->init);
    }
  } else if (const auto* var_assign_stmt = std::get_if<VarAssignStmt>(&stmt)) {
    return CheckCompExpr(sctx, func_defs, var_assign_stmt->expr);
  } else if (const auto* return_stmt = std::get_if<ReturnStmt>(&stmt)) {
    return CheckCompExpr(sctx, func_defs, return_stmt->value);
...
std::expected<void, CompError> CheckCompFunc(
  ...
  for (StmtRef stmt_ref : stmts) {
    std::expected<void, CompError> res =
        CheckCompFunc(sctx, func_defs, stmt_ref);
...
std::expected<void, CompError> CheckCompVars(
  ...
  const Stmt& stmt = sctx.DerefStmt(stmt_ref);
  if (const auto* if_stmt = std::get_if<IfStmt>(&stmt)) {
    return CheckCompVars(sctx, func_defs, if_stmt->then_stmts).and_then([&] {
      return CheckCompVars(sctx, func_defs, if_stmt->else_stmts);
    });
  } else if (const auto* loop_stmt = std::get_if<LoopStmt>(&stmt)) {
    return CheckCompVars(sctx, func_defs, loop_stmt->stmts);
  } else if (const auto* var_decl_stmt = std::get_if<VarDeclStmt>(&stmt)) {
...
std::expected<void, CompError> CheckCompVars(
  ...
  for (StmtRef stmt_ref : stmts) {
    std::expected<void, CompError> res =
        CheckCompVars(sctx, func_defs, stmt_ref);
    if (!res.has_value()) return res;
  }
  return {};
}
...
std::expected<void, CompError> CheckComp(
  ...
  std::expected<void, CompError> res =
      CheckCompVars(sctx, func_defs, stmt.stmts);
  if (res.has_value() && stmt.is_comp) {
    res = CheckCompFunc(sctx, func_defs, stmt.stmts);
  }
  return res;
}
